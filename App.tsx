...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-32">
        ...
        {/* Responsive Container: Full width for everyone on Desktop */}
        <div className="mx-auto relative z-10 max-w-md md:max-w-7xl md:px-8 md:flex md:gap-8">
            ...
            <aside className={`hidden md:block shrink-0 pt-8 sticky top-0 h-screen overflow-y-auto transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
                <div className="bg-white rounded-[24px] shadow-xl p-4 border border-slate-100 h-[calc(100vh-4rem)] flex flex-col">
                    ...
                    <div className="space-y-2 flex-1">
                            {user.role === 'murid' ? (
                            <>
                                <button onClick={() => setActiveTab('harian')} className={`w-full text-left p-3 rounded-xl font-bold text-sm flex items-center gap-3 transition-colors ${activeTab === 'harian' ? 'bg-primary-50 text-primary-600' : 'text-slate-500 hover:bg-slate-50'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Laporan Harian">
                                    <i className="fas fa-home w-6 text-center text-lg"></i> {!isSidebarCollapsed && 'Laporan Harian'}
                                </button>
                                <button onClick={() => setActiveTab('progress')} className={`w-full text-left p-3 rounded-xl font-bold text-sm flex items-center gap-3 transition-colors ${activeTab === 'progress' ? 'bg-primary-50 text-primary-600' : 'text-slate-500 hover:bg-slate-50'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Rekapitulasi">
                                    <i className="fas fa-chart-pie w-6 text-center text-lg"></i> {!isSidebarCollapsed && 'Rekapitulasi'}
                                </button>
                                <button onClick={() => setActiveTab('literasi')} className={`w-full text-left p-3 rounded-xl font-bold text-sm flex items-center gap-3 transition-colors ${activeTab === 'literasi' ? 'bg-primary-50 text-primary-600' : 'text-slate-500 hover:bg-slate-50'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Literasi">
                                    <i className="fas fa-play-circle w-6 text-center text-lg"></i> {!isSidebarCollapsed && 'Literasi'}
                                </button>
                                <button onClick={() => setActiveTab('leaderboard')} className={`w-full text-left p-3 rounded-xl font-bold text-sm flex items-center gap-3 transition-colors ${activeTab === 'leaderboard' ? 'bg-primary-50 text-primary-600' : 'text-slate-500 hover:bg-slate-50'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Peringkat">
                                    <i className="fas fa-trophy w-6 text-center text-lg"></i> {!isSidebarCollapsed && 'Peringkat'}
                                </button>
                                <button onClick={() => setActiveTab('materi')} className={`w-full text-left p-3 rounded-xl font-bold text-sm flex items-center gap-3 transition-colors ${activeTab === 'materi' ? 'bg-primary-50 text-primary-600' : 'text-slate-500 hover:bg-slate-50'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Materi">
                                    <i className="fas fa-book-open w-6 text-center text-lg"></i> {!isSidebarCollapsed && 'Materi'}
                                </button>
        ...
        {/* NEW FLOATING DOCK NAVIGATION (Mobile Only for All) */}
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[400px] h-16 nav-dock rounded-[24px] z-50 px-2 md:hidden">
